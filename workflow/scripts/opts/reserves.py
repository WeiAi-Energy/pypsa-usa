...
Snapshot coverage
-----------------
The requirement is written on every snapshot passed to ``add_ERM_constraints``,
including both representative and extreme periods. The reserve-state variables and
their capacity and energy bounds use the same complete snapshot set.
"""
...
def add_ERM_constraints(
    ...
    snapshots : pd.Index
        Snapshots on which to write the requirement. All supplied snapshots are used,
        including representative and extreme periods.
...
def store_ERM_duals(n):
    ...
    # Use the constraint's index so this also remains correct when callers explicitly
    # pass a subset of the network snapshots.
