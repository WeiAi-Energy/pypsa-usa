...
def test_erm_is_written_on_all_representative_and_extreme_snapshots():
    """Representative-period metadata does not narrow the ERM snapshot coverage."""
    n, snapshots = _energy_backed_erm_network(
        stores_weighting=5.0,
        representative_metadata=_REPRESENTATIVE_THEN_EXTREME,
    )

    expected = list(snapshots)
    assert _erm_snapshot_coverage(n, erm_requirement_name("CA_Z1")) == expected
    assert list(n.model.variables["StorageUnit-p_dispatch_ERM"].labels.indexes["snapshot"]) == expected
    assert _erm_snapshot_coverage(n, "StorageUnit-p_dispatch-soc-upper_ERM") == expected
    assert _erm_snapshot_coverage(n, "Link-p-store-upper_ERM") == expected


def test_erm_covers_every_snapshot_without_an_extreme_period():
    """Representative-only metadata still produces one requirement per snapshot."""
...
def test_erm_duals_cover_all_snapshots_with_representative_periods(reserve_margin_network):
    """Representative-period metadata does not narrow the stored ERM dual frame."""
    n = reserve_margin_network.copy()
